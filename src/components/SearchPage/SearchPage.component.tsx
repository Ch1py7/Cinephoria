...
export const SearchPage: FC = (): ReactElement => {
  ...
  const { data: moviesQuery } = useFetch<MovieTypes.Movie>(
    `search/movie?query=${query}`
  )
  const { data: seriesQuery } = useFetch<MovieTypes.Series>(
    `search/tv?query=${query}`
  )
  const { data: moviesTrending } =
    useFetch<MovieTypes.Movie>('trending/movie/day')
  const { data: seriesTrending } =
    useFetch<MovieTypes.Series>('trending/tv/day')
  ...
  return (
    <S.SearchPage>
      ...
      <h3 style={{ fontSize: '2rem', marginTop: '2rem', color: '#fff' }}>
        Explore Trending
      </h3>
      <S.MoviesContainer>
